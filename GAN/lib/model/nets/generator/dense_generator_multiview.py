...
import functools
from X2CT.GAN.lib.model.nets.generator.encoder_decoder_utils import *
# For multiview

class MultiView_UNetLike_DenseDimensionNet(nn.Module):
    def __init__(self, view1Model, view2Model, view1Order, view2Order, backToSub, decoder_output_channels,
                 decoder_out_activation, decoder_block_list=None, decoder_norm_layer=nn.BatchNorm3d,
        ...
        for index, channel in enumerate(self.decoder_channel_list[:-1]):
            ...
            decoder_layers += [
                Upsample_3DUnit(3, in_channels, out_channels, decoder_norm_layer, scale_factor=2,
        ...
        decoder_layers = []
        decoder_compress_layers = [
            nn.Conv3d(self.decoder_channel_list[0] * 2, self.decoder_channel_list[0], kernel_size=3, padding=1,
                      bias=use_bias),
        ...
        for _ in range(self.decoder_block_list[0]):
            decoder_layers += [
                nn.Conv3d(self.decoder_channel_list[0], self.decoder_channel_list[0], kernel_size=3, padding=1,
                          bias=use_bias),
                decoder_norm_layer(self.decoder_channel_list[0]),
                activation
            ]
        ...
        self.spatial_transform1_view1 = Spatial_Transform(16, view1Order, view2Order)
        self.spatial_transform1_view2 = Spatial_Transform(16, view1Order, view2Order)
        self.spatial_transform_decode = Spatial_Transform(16, view1Order, view2Order)
    ...
    def forward(self, input):
        ...
        for i in range(self.view1Model.n_downsampling):
            setattr(self.view1Model, 'feature_linker' + str(i),
                    getattr(self.view1Model, 'linker_layer' + str(i))(view1_next_input))
        ...
        view1_next_input = view1_next_input.view(view1_next_input.size(0), self.view1Model.decoder_channel_list[-1],
                                                 self.view1Model.decoder_begin_size,
        ...
        view2_next_input = view2_next_input.view(view2_next_input.size(0), self.view2Model.decoder_channel_list[-1],
                                                 self.view2Model.decoder_begin_size,
        ...
        for i in range(self.n_downsampling - 1, -2, -1):
            if i == (self.n_downsampling - 1):
                ...
                if i == -1:
                    view1_next_input = getattr(self.view1Model, 'decoder_compress_layer' + str(i))(
                        torch.cat((view1_next_input, getattr(self.view1Model, 'feature_linker' + str(i + 1))), dim=1))
                    view2_next_input = getattr(self.view2Model, 'decoder_compress_layer' + str(i))(
                        torch.cat((view2_next_input, getattr(self.view2Model, 'feature_linker' + str(i + 1))), dim=1))

                    # view1_next_input = self.spatial_transform1_view1(view1_next_input)
                    # view1_next_input = self.spatial_transform1_view2(view1_next_input)
                ...
                if self.backToSub:
                    ...
                    view1_next_input = view_avg.permute(*self.view1Order)
                    view2_next_input = view_avg.permute(*self.view2Order)
                    view_next_input = getattr(self, 'decoder_layer' + str(i))(
                        torch.cat((view_avg, view_next_input), dim=1))
                ...
                ###########
                # Last Decode for the view_next_input
                ###########
        out_view_next_input = self.decoder_layer(view_next_input)
        out_view_next_input = self.spatial_transform_decode(out_view_next_input, view1_next_input, view2_next_input)
        return self.view1Model.decoder_layer(view1_next_input), self.view2Model.decoder_layer(
            view2_next_input), out_view_next_input


def UNetLike_DownStep5(input_shape, encoder_input_channels, decoder_output_channels, decoder_out_activation,
                       encoder_norm_layer, decoder_norm_layer, upsample_mode, decoder_feature_out=False):
    # 64, 32, 16, 8, 4
    ...
    return UNetLike_DenseDimensionNet(encoder_input_channels, decoder_output_channels, decoder_begin_size,
                                      encoder_block_list, decoder_block_list, growth_rate, encoder_channel_list,
                                      decoder_channel_list, decoder_out_activation, encoder_norm_layer,
                                      decoder_norm_layer, upsample_mode, decoder_feature_out)

def UNetLike_DownStep5_attention(input_shape, encoder_input_channels, decoder_output_channels, decoder_out_activation,
                       encoder_norm_layer, decoder_norm_layer, upsample_mode, decoder_feature_out=False):
    # 64, 32, 16, 8, 4
    encoder_block_list = [6, 12, 24, 16, 6]
    decoder_block_list = [1, 2, 2, 2, 2, 0]
    ...
    decoder_channel_list = [16, 16, 32, 64, 128, 256]
    decoder_begin_size = input_shape // pow(2, len(encoder_block_list))
    return UNetLike_DenseDimensionNet_attention(encoder_input_channels, decoder_output_channels, decoder_begin_size,
                                      encoder_block_list, decoder_block_list, growth_rate, encoder_channel_list,
                                      decoder_channel_list, decoder_out_activation, encoder_norm_layer,
                                      decoder_norm_layer, upsample_mode, decoder_feature_out)

def UNetLike_DownStep5_sft(input_shape, encoder_input_channels, decoder_output_channels, decoder_out_activation,
                       encoder_norm_layer, decoder_norm_layer, upsample_mode, decoder_feature_out=False):
    # 64, 32, 16, 8, 4
    encoder_block_list = [6, 12, 24, 16, 6]
    decoder_block_list = [1, 2, 2, 2, 2, 0]
    ...
    decoder_channel_list = [16, 16, 32, 64, 128, 256]
    decoder_begin_size = input_shape // pow(2, len(encoder_block_list))
    return UNetLike_DenseDimensionNet_sft(encoder_input_channels, decoder_output_channels, decoder_begin_size,
                                      encoder_block_list, decoder_block_list, growth_rate, encoder_channel_list,
                                      decoder_channel_list, decoder_out_activation, encoder_norm_layer,
                                      decoder_norm_layer, upsample_mode, decoder_feature_out)


def UNetLike_DownStep5_3(input_shape, encoder_input_channels, decoder_output_channels, decoder_out_activation,
                         encoder_norm_layer, decoder_norm_layer, upsample_mode, decoder_feature_out=False):
    # 64, 32, 16, 8, 4
...
class ResNetLike_DenseDimensionNet(nn.Module):
  def __init__(self, encoder_input_channels, decoder_output_channels, decoder_begin_size, encoder_block_list, decoder_block_list, growth_rate, encoder_channel_list, decoder_channel_list, decoder_out_activation, encoder_norm_layer=nn.BatchNorm2d, decoder_norm_layer=nn.BatchNorm3d, upsample_mode='nearest', decoder_feature_out=False):
    super(UNetLike_DenseDimensionNet, self).__init__()
    print('Normal Encoder Decoder')
    self.decoder_channel_list = decoder_channel_list
...
class UNetLike_DenseDimensionNet_attention(nn.Module):
    def __init__(self, encoder_input_channels, decoder_output_channels, decoder_begin_size, encoder_block_list,
                 decoder_block_list, growth_rate, encoder_channel_list, decoder_channel_list, decoder_out_activation,
                 encoder_norm_layer=nn.BatchNorm2d, decoder_norm_layer=nn.BatchNorm3d, upsample_mode='nearest',
                 decoder_feature_out=False):
        super(UNetLike_DenseDimensionNet_attention, self).__init__()
        print('Encoder Decoder with attention')
        self.decoder_channel_list = decoder_channel_list
        self.decoder_block_list = decoder_block_list
        self.n_downsampling = len(encoder_block_list)
        self.decoder_begin_size = decoder_begin_size
        ...
        for index, channel in enumerate(encoder_block_list):
            ...
            down_layers += [
                # With CA layer in the middle
                Dense_2DBlock_attention(encoder_block_list[index], num_input_channels, bn_size, growth_rate, encoder_norm_layer,
                              activation, use_bias),
            ]
        ...
        for index, channel in enumerate(encoder_channel_list[:-1]):
            ...
            out_channels = decoder_channel_list[index]
            link_layers = [
                Dimension_UpsampleCutBlock(in_channels, out_channels, encoder_norm_layer, decoder_norm_layer,
                                           activation, use_bias)
            ]
            setattr(self, 'linker_layer' + str(index), nn.Sequential(*link_layers))
        ...
        for index, channel in enumerate(decoder_channel_list[:-1]):
            ...
            if index != (len(decoder_channel_list) - 2):
                decoder_compress_layers += [
                    ...
                    CALayer_3D(in_channels, reduction=(in_channels // 2)), # Channel Attention 3D
            ...
            decoder_layers += [
                # With 3D CA
                Upsample_3DUnit(3, in_channels, out_channels, decoder_norm_layer, scale_factor=2,
                                upsample_mode=upsample_mode, activation=activation, use_bias=use_bias)
            ]
            # If decoder_feature_out is True, compressed feature after upsampling and concatenation
            # can be obtained.
            if decoder_feature_out:
        ...
        decoder_compress_layers = [
            ...
            # CALayer_3D( decoder_channel_list[0], reduction=( decoder_channel_list[0] // 2)),  # Channel Attention 3D
    ...
    def forward(self, input):
        ...
        next_input = self.base_link(next_input.view(next_input.size(0), -1))
        next_input = next_input.view(next_input.size(0), self.decoder_channel_list[-1], self.decoder_begin_size,
                                     self.decoder_begin_size, self.decoder_begin_size)


        for i in range(self.n_downsampling - 1, -2, -1):
            if i == (self.n_downsampling - 1):
                if self.decoder_feature_out:
                    next_input = getattr(self, 'decoder_layer' + str(i))(
                        getattr(self, 'decoder_compress_layer' + str(i))(next_input))
                else:
                    next_input = getattr(self, 'decoder_layer' + str(i))(next_input)
                ...
                if self.decoder_feature_out:
                    next_input = getattr(self, 'decoder_layer' + str(i))(
                        getattr(self, 'decoder_compress_layer' + str(i))(
                            torch.cat((next_input, getattr(self, 'feature_linker' + str(i + 1))), dim=1)))
                else:
                    next_input = getattr(self, 'decoder_layer' + str(i))(
                        torch.cat((next_input, getattr(self, 'feature_linker' + str(i + 1))), dim=1))
...
class UNetLike_DenseDimensionNet_sft(nn.Module):
    def __init__(self, encoder_input_channels, decoder_output_channels, decoder_begin_size, encoder_block_list,
                 decoder_block_list, growth_rate, encoder_channel_list, decoder_channel_list, decoder_out_activation,
                 encoder_norm_layer=nn.BatchNorm2d, decoder_norm_layer=nn.BatchNorm3d, upsample_mode='nearest',
                 decoder_feature_out=False):
        super(UNetLike_DenseDimensionNet_sft, self).__init__()
        print('Encoder Decoder with sft layer')
        self.decoder_channel_list = decoder_channel_list
        self.decoder_block_list = decoder_block_list
        self.n_downsampling = len(encoder_block_list)
        self.decoder_begin_size = decoder_begin_size
        ...
        # Conditional Net
        ##############
        self.CondNet_2d = nn.Sequential(
            nn.Conv2d(1, 128, kernel_size=3, padding=1), nn.LeakyReLU(0.1, True), nn.Conv2d(128, 128, 1),
        ...
        for index, channel in enumerate(encoder_block_list):
            ...
            setattr(self, 'encoder_layer_part1' + str(index), nn.Sequential(*down_layers))
            down_layers = []

            down_layers = [
                # With CA layer in the middle
                Dense_2DBlock_sft(encoder_block_list[index], num_input_channels, bn_size, growth_rate, encoder_norm_layer,
                              activation, use_bias),
            ]

            setattr(self, 'encoder_layer_part2' + str(index), nn.Sequential(*down_layers))
            down_layers = []
            ...
            if index == (self.n_downsampling - 1):
                ...
                num_input_channels = num_out_channels
            encoder_channel_list.append(num_input_channels)
            setattr(self, 'encoder_layer_part3' + str(index), nn.Sequential(*down_layers))
    ...
    def forward(self, input):
        # xray = input[0]
        # seg = input[1]
        cond = self.CondNet_2d(input[1])
        encoder_feature = self.encoder_layer(input[0])
        next_input = encoder_feature
        # print(cond.shape, next_input.shape)
        for i in range(self.n_downsampling):
            if i == 0:
                # print(next_input.shape,"0")
                setattr(self, 'feature_linker' + str(i), getattr(self, 'linker_layer' + str(i))(next_input))
                next_input = getattr(self, 'encoder_layer_part1' + str(i))(next_input)
                cond = getattr(self, 'encoder_layer_part1' + str(i))(cond)
            else:
                # print(tmp_out[0].shape,'1')
                setattr(self, 'feature_linker' + str(i), getattr(self, 'linker_layer' + str(i))(next_input))
                next_input = getattr(self, 'encoder_layer_part1' + str(i))(next_input)
                cond = getattr(self, 'encoder_layer_part1' + str(i))(next_cond)
            # print(next_input.shape, cond.shape)

            batch = (next_input, cond)
            tmp_batch = getattr(self, 'encoder_layer_part2' + str(i))(batch)

            # print(next_input[0].shape,'?')
            # print(next_input[1].shape,'fu')

            next_input = getattr(self, 'encoder_layer_part3' + str(i))(tmp_batch[0])
            next_cond = getattr(self, 'encoder_layer_part3' + str(i))(tmp_batch[1])
            # print(next_input.shape,'la')
